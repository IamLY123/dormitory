<template>
    <app-layout title="EditWater">
        <template #header>
            <h2>แก้ไขข้อมูลค่านำ้</h2>
        </template>
                <div class="py-6">
            <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-3">
                    <div v-for="(p) in water" v-bind:value="p.id">
                    <form @submit.prevent="submit(p.id)">
                    <div v-bind:value="p.id" class="py-3">
                        <h1>เลขห้อง: {{p.roomrent.room.room_number}}</h1>
                        <h1>จำนวนที่ใช้ต่อเดือน: {{p.power_use}} หน่วย </h1>
                        <h1>ราคาต่อหน่วย: {{p.unit}} บาท </h1>
